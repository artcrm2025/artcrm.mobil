-- Hata düzeltilmiş v2 - Auth sistemine kayıt olan kullanıcıları otomatik olarak users tablosuna ekleyen trigger
...
BEGIN
  ...
  BEGIN
    ...
    FOR auth_user IN 
      ...
      FROM auth.users
    LOOP
      -- Her kullanıcı için başarılı başlar
      success := TRUE;
      error_message := NULL;
      
      -- Güvenli bir blok içinde işlemi gerçekleştir
      BEGIN
        -- Kullanıcı adını hazırla
      ...
      EXCEPTION WHEN OTHERS THEN
        -- Hata durumunda hata mesajını kaydet ve devam et
        success := FALSE;
        error_message := SQLERRM;
      END;
      
      -- İşlem başarısız olduysa loglama yap
      IF NOT success THEN
        RAISE NOTICE 'Error updating user % : %', auth_user.email, error_message;
      END IF;
    END LOOP;
